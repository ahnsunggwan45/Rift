...
pub struct RaknetSocket {
    ...
    user_data_receiver: Arc<Mutex<Receiver<Bytes>>>,
...
impl RaknetSocket {
    ...
    pub async fn from(
        ...
        let (user_data_sender, user_data_receiver) = channel::<Bytes>(100);
            ...
            loop {
                    ...
                    && buf[0] <= PacketID::FrameSetPacketEnd.to_u8()
                {
                    ...
                    let frames = match FrameVec::new(buf.clone()) {
                        Ok(f) => f,
                        Err(_) => {
                            raknet_log_debug!("malformed frameset dropped");
                            continue;
                        }
                    };
                    ...
                    for frame in frames.frames {
                        ...
                        for f in recvq.flush(cur_timestamp_millis(), &peer_addr) {
                            ...
                            .await
                            {
                                ...
                                Err(_) => raknet_log_debug!("frame handle error, dropped"),
                            }
                        }
        ...
        tokio::spawn(async move {
            loop {
                ...
                if !acks.is_empty() {
                    ...
                    let buf = write_packet_ack(&ack).unwrap();
                    RaknetSocket::sendto(
                        &s,
                        &buf,
                        &peer_addr,
    ...
    /// Ordered-delivery diagnostics for stall watching:
    /// `(next_ordered_index, ordered_backlog, ordered_delivered, ordered_dropped_behind, sendq_unacked)`.
