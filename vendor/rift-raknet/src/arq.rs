...
impl FrameSetPacket {
    pub fn new(r: Reliability, data: impl Into<Bytes>) -> FrameSetPacket {
        let data = data.into();
        let flag = r.to_u8() << 5;
    ...
    pub fn _deserialize(buf: Vec<u8>) -> Result<(Self, bool)> {
        ...
        ret.length_in_bytes = reader.read_u16(Endian::Big)? / 8;
    ...
    pub fn serialize(&self) -> Result<Vec<u8>> {
        ...
        writer.write(self.data.as_ref()).unwrap();
...
impl FrameVec {
    pub fn new(buf: Vec<u8>) -> Result<Self> {
        ...
        while reader.pos() < size.try_into().unwrap() {
            ...
            frame.length_in_bytes = reader.read_u16(Endian::Big)? / 8;
    ...
    pub fn _serialize(&self) -> Result<Vec<u8>> {
        ...
        for frame in &self.frames {
            ...
            writer.write(frame.data.as_ref()).unwrap();
        }
    ...
    pub fn insert(&mut self, frame: FrameSetPacket) -> Result<()> {
        ...
        if matches!(
            ...
            if self.reliable_window.len() > RELIABLE_DEDUP_WINDOW {
                if let Some(old) = self.reliable_window.pop_front() {
                    self.received_reliable.remove(&old);
                }
            }
        }
    ...
    pub fn insert_bytes(&mut self, reliability: Reliability, data: Bytes) -> Result<()> {
        match reliability {
            Reliability::Unreliable => {
                ...
                if data.len() > (self.mtu - 60).into() {
                    return Err(RaknetError::PacketSizeExceedMTU);
                }

                let frame = FrameSetPacket::new(reliability, data.clone());
            ...
            Reliability::UnreliableSequenced => {
                ...
                let mut frame = FrameSetPacket::new(reliability, data.clone());
            ...
            Reliability::ReliableOrdered => {
                if data.len() < (self.mtu - 60).into() {
                    ...
                    let mut compound_size = data.len() / max;
                    if data.len() % max != 0 {
                        compound_size += 1;
                    }
